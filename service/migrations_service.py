...
async def load_bulk_activities_data(data: ActivitiesMigrationsRequestSchema, user: User) -> None:
    """
    Load bulk activities data to SQS queue.

    :param data: migration data from request payload
    :param user: user object that requested the migration

    :return: None
    """
    activities_migration_payload = data.model_dump()
    activities_migration_payload['user_id'] = str(user.id)
