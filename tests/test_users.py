def test_read_user_with_token(client, authenticated_user):
    response = client.get("/v1/users/", headers={"Authorization": f"Bearer {authenticated_user.current_token}"})
...
def test_read_user_unauthenticated_token(client, unauthenticated_user):
    response = client.get("/v1/users/", headers={"Authorization": f"Bearer {unauthenticated_user.current_token}"})
