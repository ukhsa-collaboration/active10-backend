...
async def create_daily_target(
    ...
    existing_target = daily_target_crud.get_user_target_by_payload_data(user_id=user.id, data=payload)

    if existing_target:
        raise HTTPException(status_code=400, detail="User daily target with same data already exists")
...
async def get_user_daily_targets_list(
        user: Annotated[User, Depends(get_authenticated_user_data)],
        daily_target_crud: Annotated[UserDailyTargetCRUD, Depends()],
        date: Optional[int] = Query(None, description="Filter by exact date (UNIX timestamp)"),
        ...
        min_daily_target: Optional[int] = Query(None, description="Filter by minimum daily target"),
        max_daily_target: Optional[int] = Query(None, description="Filter by maximum daily target")
):
    filters = {
        "date": date,
...
async def get_user_daily_target(
    ...
    daily_target = daily_target_crud.get_user_daily_target_by_id(user_id=user.id, target_id=target_id)
...
async def update_daily_target(
    ...
    user_daily_target = daily_target_crud.get_user_daily_target_by_id(user_id=user.id, target_id=target_id)
