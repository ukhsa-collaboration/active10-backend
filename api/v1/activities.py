...
async def list_activities(
    ...
    if date and (start_date or end_date):
        raise HTTPException(status_code=400, detail="Cannot use date filter with start_date or end_date filters")

    if (start_date and not end_date) or (end_date and not start_date):
        raise HTTPException(status_code=400, detail="Both start_date and end_date filters must be used together")
    ...
    if not date and not start_date and not end_date:
        start_date = int((datetime.now(timezone.utc).replace(tzinfo=None) - relativedelta(years=1)).timestamp())
